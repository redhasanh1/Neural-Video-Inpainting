...
# Apple puts the bundle id of the app that asked for the token in the token's
# `aud` claim, and it has to be listed here or the sign-in is rejected. Both ids
# are accepted: the App Store build ships as com.markremoverai.objectremover,
# while TestFlight builds of the original app are still com.markremoverai.app.
APPLE_SIGNIN_BUNDLE_IDS = [
    b.strip() for b in os.getenv(
        'APPLE_SIGNIN_BUNDLE_IDS',
        'com.markremoverai.app,com.markremoverai.objectremover'
    ).split(',') if b.strip()
]
